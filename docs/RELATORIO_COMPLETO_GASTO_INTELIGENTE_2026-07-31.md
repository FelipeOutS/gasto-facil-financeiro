...
|---|---|---|---|
| S1 | ~~Vulnerabilidade critica em dependencias: `seroval` via `@tanstack/*` (GHSA-mv8w-475r-vwqw / CVE-2026-59940)~~ - **CORRIGIDA LOCALMENTE, PUBLICADA E VALIDADA EM PRODUCAO (2026-07-31, ~19:28 UTC)**: `seroval` e `seroval-plugins` travados em **1.5.6** (faixa vulneravel ate 1.5.2) via `overrides`, com lockfile de texto `bun.lock` versionado. Finding critico de supply chain encerrado (0 criticos / 0 altos / 0 medios / 0 baixos apos o deploy); 8 rotas publicas e 13 rotas de app com HTTP 200 em producao. Detalhes em `docs/CORRECAO_CVE_SEROVAL_GASTO_INTELIGENTE_2026-07-31.md` | **Resolvido em producao** | arvore real + teste automatizado + smoke em producao |
