...
export function MoreSheet({ open, onOpenChange }: Props) {
  ...
  useEffect(() => {
    ...
    window.history.pushState({ ...(window.history.state ?? {}), moreMenuOpen: true }, "", window.location.href);
  ...
  const menuPortal = portalReady && open
    ? createPortal(
        <>
          <div
          ...
          <section
            ...
            style={{
              ...
              maxHeight: "calc(100dvh - 104px - env(safe-area-inset-top, 0px) - env(safe-area-inset-bottom, 0px))",
            }}
          >
