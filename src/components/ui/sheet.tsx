...
const sheetVariants = cva("fixed z-[10000] gap-4 bg-background p-6 shadow-2xl shadow-black/50", {
  variants: {
    side: {
      top: "inset-x-0 top-0 max-h-[calc(100dvh-1rem)] overflow-y-auto overscroll-contain border-b",
      bottom:
        "inset-x-0 bottom-0 max-h-[calc(100dvh-1rem)] overflow-y-auto overscroll-contain border-t pb-[max(1.5rem,env(safe-area-inset-bottom))]",
      left: "inset-y-0 left-0 h-full w-3/4 max-w-[calc(100vw-2rem)] overflow-y-auto overscroll-contain border-r sm:max-w-sm",
      right:
        "inset-y-0 right-0 h-full w-3/4 max-w-[calc(100vw-2rem)] overflow-y-auto overscroll-contain border-l sm:max-w-sm",
    },
  },
  defaultVariants: {
    side: "right",
