...
export const GoogleMapView = React.forwardRef<GoogleMapViewHandle, GoogleMapViewProps>(
  function GoogleMapView({ height = 320, className, markers }, ref) {
    ...
    if (state.status === "error") {
      const isKeyMissing =
        typeof state.message === "string" && state.message.toLowerCase().includes("chave");
      ...
      return (
        ...
        >
          ...
          <p className="max-w-xs text-sm text-muted-foreground">
            {isKeyMissing ? t("map.keyMissingDescription") : t("map.loadErrorDescription")}
          </p>
          <button
            type="button"
            onClick={init}
    ...
    return (
      <div
        ref={containerRef}
        className={cn("rounded-3xl border border-border/60 bg-card overflow-hidden", className)}
        style={{ height: heightStyle }}
