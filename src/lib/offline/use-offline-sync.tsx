...
export function syncAllForUser(actorId: string): Promise<{ synced: number; failed: number }> {
  ...
  const task = (async () => {
    let synced = 0,
      failed = 0;
    for (const candidate of await listExpenses(actorId, actorId, true)) {
      if (typeof navigator !== "undefined" && navigator.onLine === false) break;
      try {
        await withSyncAttempt("gf:expense:" + candidate.local_id, async (signal) => {
          const item = await claimForSync(candidate.local_id, candidate.user_id, actorId);
          if (!item || signal.aborted) return;
          try {
            const result = await addGastoAwait(
              item.input,
              item.user_id,
              item.local_id,
              actorId,
            ...
            await deleteExpenseSilent(item.local_id, item.user_id, item.attempts, actorId);
            synced++;
...
export function useOfflineExpenseQueue(ownerId: string | null | undefined, actorId = ownerId) {
  ...
  const refresh = useCallback(async () => {
    ...
    const list = await listExpenses(ownerId, actorId);
  ...
  const syncNow = useCallback(async () => {
    ...
    const result = await syncAllForUser(actorId);
    await refresh();
    return result;
  }, [actorId, refresh]);
