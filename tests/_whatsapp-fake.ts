...
function makeBuilder(table: string): any {
  ...
  const finalize = async () => {
    ...
    if (ctx.op === "insert") {
      ...
      for (const r of rows) {
        const newRow = { ...r, id: r.id || `m-${state.inserts.length + 1}` };
      ...
      // Readback Guard espera data[0] se for objeto, ou array.
    ...
    if (table === "whatsapp_links") {
      return { data: [{ user_id: "u1", ativo: true, opt_in_em: new Date().toISOString(), revogado_em: null }], error: null };
    }
...
export const fakeAdmin = {
  ...
  rpc: async (n: string, a: any) => {
    ...
    if (n === "has_feature_access" || n === "can_use_whatsapp" || n === "has_role" || n === "is_full_access") return { data: true };
    return { data: true };
  },
...
mock.module("@/server/whatsapp-entitlement.server", () => ({ getWhatsAppEntitlement: async () => ({ allowed: true, plan: "pessoal_premium", planActive: true, featureIncluded: true, betaAllowed: true, linkActive: true, optInActive: true, checkedAt: new Date().toISOString() }), assertWhatsAppEntitlement: async () => ({ allowed: true }) }));
