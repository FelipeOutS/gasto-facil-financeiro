 ...
 *  - isolamento por user_id (chamadas usam IDs distintos);
 *  - logs seguros (sem PII).
 *
 * Não toca em Supabase real: mocka `supabaseAdmin` via `bun:test` mock.
 */
...
function makeSupabaseMock() {
  return {
    from(table: string) {
      if (table !== "whatsapp_merchant_category_memories") {
        throw new Error(`unexpected table ${table}`);
      }
      let filter: Partial<MemRow> = {};
      const query: Record<string, unknown> = {};
      query.select = () => query;
      query.eq = (col: keyof MemRow, val: unknown) => {
        (filter as Record<string, unknown>)[col as string] = val;
        return query;
      };
      ...
      // Pretend `await query` resolves to a list (for the lookup path).
      query.then = (resolve: (v: { data: MemRow[]; error: null }) => unknown) => {
        const list = rows.filter((x) =>
          Object.entries(filter).every(([k, v]) => (x as Record<string, unknown>)[k] === v),
        );
        return Promise.resolve(resolve({ data: list, error: null }));
      };
      query.insert = async (payload: Partial<MemRow>) => {
        rows.push({
      ...
      query.update = (patch: Partial<MemRow>) => {
        return {
          eq: async () => {
            const r = rows.find((x) =>
              Object.entries(filter).every(([k, v]) => (x as Record<string, unknown>)[k] === v),
...
} from "@/server/whatsapp-merchant-memory.server";
...
describe("merchantKeyFor — normalização", () => {
  it("normaliza acentos, caixa e pontuação", () => {
    expect(merchantKeyFor("Caboclo Ventania Artigos de Fé"))
      .toBe("caboclo ventania artigos de fe");
  });
  it("remove pontuação periférica e asteriscos", () => {
    expect(merchantKeyFor("UBER *TRIP  123")).toBe("uber trip");
  });
  it("preserva palavras relevantes", () => {
    expect(merchantKeyFor("Drogaria São Paulo")).toBe("drogaria sao paulo");
  });
  it("rejeita descrição genérica monolítica", () => {
    expect(merchantKeyFor("gasto")).toBeNull();
    expect(merchantKeyFor("almoço")).toBeNull();
    expect(merchantKeyFor("mercado")).toBeNull();
  });
  it("rejeita descrição muito curta", () => {
    expect(merchantKeyFor("a")).toBeNull();
    expect(merchantKeyFor("ab")).toBeNull();
  });
  it("rejeita identificador puramente numérico", () => {
    expect(merchantKeyFor("12345")).toBeNull();
  });
  it("rejeita combinação só de termos genéricos", () => {
    expect(merchantKeyFor("compra pagamento")).toBeNull();
  });
});
...
describe("lookupMerchantMemory — elegibilidade", () => {
  it("uma confirmação automática não basta", async () => {
    ...
    const r = await lookupMerchantMemory({ userId: "u1", merchantKey: "caboclo ventania", activeCategoryIds: activeIds });
  ...
  it("histórico conflitante (2 categorias elegíveis) é ambíguo", async () => {
    rows.push({ user_id: "u1", merchant_key: "caboclo ventania", category_id: "cat-presentes", confirmed_count: 2, manual_confirmed_count: 0 });
  ...
  it("categoria inativa é ignorada", async () => {
    rows.push({ user_id: "u1", merchant_key: "x", category_id: "cat-inativa", confirmed_count: 5, manual_confirmed_count: 5 });
    const r = await lookupMerchantMemory({ userId: "u1", merchantKey: "x", activeCategoryIds: activeIds });
    expect(r.kind).toBe("none");
  });
...
describe("recordMerchantMemory — escrita", () => {
  it("cria registro novo na primeira gravação", async () => {
    const r = await recordMerchantMemory({ userId: "u1", merchantKey: "k", categoryId: "cat-presentes", evidence: "manual" });
  ...
  it("incrementa contadores em gravações subsequentes", async () => {
    await recordMerchantMemory({ userId: "u1", merchantKey: "k", categoryId: "cat-presentes", evidence: "confirmed" });
    await recordMerchantMemory({ userId: "u1", merchantKey: "k", categoryId: "cat-presentes", evidence: "manual" });
    expect(rows.length).toBe(1);
  ...
  it("não grava sem categoria", async () => {
    const r = await recordMerchantMemory({ userId: "u1", merchantKey: "k", categoryId: "", evidence: "confirmed" });
    expect(r.ok).toBe(false);
    expect(rows.length).toBe(0);
  });

  it("não grava sem merchant key", async () => {
    const r = await recordMerchantMemory({ userId: "u1", merchantKey: "", categoryId: "cat-presentes", evidence: "confirmed" });
    expect(r.ok).toBe(false);
  });
});
...
describe("logMerchantMemoryDecision — sem PII", () => {
  it("log contém apenas decisão (sem userId, key, categoria, valor)", () => {
    ...
    // Garantia explícita: nenhum campo sensível.
