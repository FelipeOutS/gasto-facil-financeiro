...
async function setup(props: { min: number; max: number; fallback?: number; initial: number }) {
...
describe("IntegerInput — dia do cartão (1..31)", () => {
  it("permite digitar 24 sem o sistema alterar o valor", async () => {
    const { input, out, type, blur } = await setup({ min: 1, max: 31, initial: 1 });
    type("2");
  ...
  it("campo pode ficar vazio durante a edição", async () => {
    const { input, type } = await setup({ min: 1, max: 31, initial: 31 });
    type("");
    expect(input.value).toBe("");
  });

  it("vazio no blur cai no fallback do campo", async () => {
    const { input, out, type, blur } = await setup({
      min: 1,
      max: 31,
      fallback: 10,
      initial: 24,
  ...
  it("rejeita 0, 32 e 99 no blur (clamp para a faixa)", async () => {
    for (const [entrada, esperado] of [
      ...
      const { input, type, blur } = await setup({ min: 1, max: 31, initial: 5 });
      type(entrada);
      blur();
  ...
  it("ignora caracteres não numéricos", async () => {
    const { input, type } = await setup({ min: 1, max: 31, initial: 1 });
    type("2a4");
    expect(input.value).toBe("24");
  });
});
...
describe("IntegerInput — recorrência em meses (1..60)", () => {
  it("aceita 6 meses (caso reportado) e 1..12", async () => {
    for (let m = 1; m <= 12; m++) {
      const { input, out, type, blur } = await setup({ min: 1, max: 60, fallback: 12, initial: 12 });
      type(String(m));
  ...
  it("não usa faixa de dia do mês: 60 meses é válido", async () => {
    const { input, type, blur } = await setup({ min: 1, max: 60, initial: 12 });
    type("60");
    blur();
...
describe("IntegerInput — parcelas (2..36)", () => {
  it("permite digitar 6 parcelas e limita acima de 36 só no blur", async () => {
    const { input, type, blur } = await setup({ min: 2, max: 36, fallback: 2, initial: 2 });
    type("6");
