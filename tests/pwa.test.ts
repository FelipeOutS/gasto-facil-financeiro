...
describe('PWA 8B Validation', () => {
  ...
  it('should have a secure offline page', () => {
    const offlineContent = fs.readFileSync(path.resolve(process.cwd(), 'public/offline.html'), 'utf-8');
    expect(offlineContent).toContain('lang="pt-BR"');
    expect(offlineContent).toContain('Reconecte-se para acessar e atualizar seus dados financeiros com segurança');
  });
});
